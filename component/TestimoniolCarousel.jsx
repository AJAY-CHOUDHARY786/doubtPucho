import React from "react";
import Carousel from "react-multi-carousel";
import "react-multi-carousel/lib/styles.css";
const TestimoniolCarousel = () => {
  const responsive = {
    superLargeDesktop: {
      // the naming can be any, depends on you.
      breakpoint: { max: 4000, min: 3000 },
      items: 1,
    },
    desktop: {
      breakpoint: { max: 3000, min: 1024 },
      items: 1,
    },
    tablet: {
      breakpoint: { max: 1024, min: 464 },
      items: 1,
    },
    mobile: {
      breakpoint: { max: 464, min: 0 },
      items: 1,
    },
  };

  return (
    <div className="TestiMainCont">
      <Carousel responsive={responsive}>
        <div className="TestiSubMainCont">
          <div className="Item1">
            <div className="itemimage">
              <img
                src="https://static-cf.toppr.com/marketing/6c448a3/images/customer-stories/govind-menon.jpg"
                alt=""
              />
            </div>
            <div className="itemimagecont">
             
              <h2> AJAY CHOUDHARY </h2>
              <h3>Jee</h3>
            
              <p> Lorem, ipsum fhhhhhh hhgisrh fhfvnje rhjdrgih dfgvns jfnvsf viudjf bndg bhvd of jbod
                jshv svjdfvi jndjnvj dkjvnjdf vldflvjjf vnkdfvdn sgivudfo gnvdf nibh vdnbjd gbn dg ckb.</p>{" "}
          
              
     
            </div>
          </div>
          <div className="Item1">
            <div className="itemimage">
              <img
                src="https://static-cf.toppr.com/marketing/6c448a3/images/customer-stories/govind-menon.jpg"
                alt=""
              />
            </div>
            <div className="itemimagecont">
              {" "}
              <h2> Ajay </h2>
              <h3>Jee</h3>
              <p> Lorem, ipsum fhhhhhh hhgisrh fhfvnje rhjdrgih dfgvns jfnvsf viudjf bndg bhvd of jbod
                jshv svjdfvi jndjnvj dkjvnjdf vldflvjjf vnkdfvdn sgivudfo gnvdf nibh vdnbjd gbn dg ckb.</p>{" "}
            </div>
          </div>
        </div>
        <div>
          <div className="Item1">
            <div className="itemimage">
              <img
                src="https://static-cf.toppr.com/marketing/6c448a3/images/customer-stories/govind-menon.jpg"
                alt=""
              />
            </div>
            <div className="itemimagecont">
              {" "}
              <h2> Ajay </h2>
              <h3>Jee</h3>
              <p> Lorem, ipsum fhhhhhh hhgisrh fhfvnje rhjdrgih dfgvns jfnvsf viudjf bndg bhvd of jbod
                jshv svjdfvi jndjnvj dkjvnjdf vldflvjjf vnkdfvdn sgivudfo gnvdf nibh vdnbjd gbn dg ckb.</p>{" "}
            </div>
          </div>
          <div className="Item1">
            <div className="itemimage">
              <img
                src="https://static-cf.toppr.com/marketing/6c448a3/images/customer-stories/govind-menon.jpg"
                alt=""
              />
            </div>
            <div className="itemimagecont">
              {" "}
              <h2> Ajay </h2>
              <h3>Jee</h3>
              <p> Lorem, ipsum fhhhhhh hhgisrh fhfvnje rhjdrgih dfgvns jfnvsf viudjf bndg bhvd of jbod
                jshv svjdfvi jndjnvj dkjvnjdf vldflvjjf vnkdfvdn sgivudfo gnvdf nibh vdnbjd gbn dg ckb.</p>{" "}
            </div>
          </div>
        </div>
        <div>
          <div className="Item1">
            <div className="itemimage">
              <img
                src="https://static-cf.toppr.com/marketing/6c448a3/images/customer-stories/govind-menon.jpg"
                alt=""
              />
            </div>
            <div className="itemimagecont">
              {" "}
              <h2> Ajay </h2>
              <h3>Jee</h3>
              <p> Lorem, ipsum fhhhhhh hhgisrh fhfvnje rhjdrgih dfgvns jfnvsf viudjf bndg bhvd of jbod
                jshv svjdfvi jndjnvj dkjvnjdf vldflvjjf vnkdfvdn sgivudfo gnvdf nibh vdnbjd gbn dg ckb.</p>{" "}
            </div>
          </div>
          <div className="Item1">
            <div className="itemimage">
              <img
                src="https://static-cf.toppr.com/marketing/6c448a3/images/customer-stories/govind-menon.jpg"
                alt=""
              />
            </div>
            <div className="itemimagecont">
              {" "}
              <h2> Ajay </h2>
              <h3>Jee</h3>
              <p> Lorem, ipsum fhhhhhh hhgisrh fhfvnje rhjdrgih dfgvns jfnvsf viudjf bndg bhvd of jbod
                jshv svjdfvi jndjnvj dkjvnjdf vldflvjjf vnkdfvdn sgivudfo gnvdf nibh vdnbjd gbn dg ckb.</p>{" "}
            </div>
          </div>
        </div>
        <div>
        
          <div className="Item1">
            <div className="itemimage">
              <img
                src="https://static-cf.toppr.com/marketing/6c448a3/images/customer-stories/govind-menon.jpg"
                alt=""
              />
            </div>
            <div className="itemimagecont">
              {" "}
              <h2> Ajay </h2>
              <h3>Jee</h3>
              <p> Lorem, ipsum fhhhhhh hhgisrh fhfvnje rhjdrgih dfgvns jfnvsf viudjf bndg bhvd of jbod
                jshv svjdfvi jndjnvj dkjvnjdf vldflvjjf vnkdfvdn sgivudfo gnvdf nibh vdnbjd gbn dg ckb.</p>{" "}
            </div>
            
          </div>
          <div className="Item1">
            <div className="itemimage">
              <img
                src="https://static-cf.toppr.com/marketing/6c448a3/images/customer-stories/govind-menon.jpg"
                alt=""
              />
            </div>
            <div className="itemimagecont">
              {" "}
              <h2> Ajay </h2>
              <h3>Jee</h3>
              <p> Lorem, ipsum fhhhhhh hhgisrh fhfvnje rhjdrgih dfgvns jfnvsf viudjf bndg bhvd of jbod
                jshv svjdfvi jndjnvj dkjvnjdf vldflvjjf vnkdfvdn sgivudfo gnvdf nibh vdnbjd gbn dg ckb.</p>{" "}
            </div>
          </div>
        </div>
      </Carousel>
    </div>
  );
};

export default TestimoniolCarousel;
